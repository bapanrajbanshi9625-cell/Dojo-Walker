...
class _LiveWalkScreenState extends State<LiveWalkScreen> {
  ...
  static const Color cardBorder = Color(0xFFE1E6E8);
  ...
  DocumentReference<Map<String, dynamic>> get _activeWalkRef =>
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (
        ...
        final String status =
            activeData['status']?.toString().toLowerCase() ?? 'active';

        if (status != 'active') {
          return _completedScreen(activeData);
        }
  ...
  Widget _activeWalkScreen(
    ...
    final int elapsedSeconds = _toInt(
      sessionData['elapsedSeconds'],
    );

    final String duration =
        sessionData['elapsedSeconds'] != null
            ? _formatDuration(elapsedSeconds)
            : activeData['duration']?.toString() ?? '00:00:00';

    // ----------------------------------------------------------
    // DISTANCE
    // ----------------------------------------------------------

    final double distanceKm = _toDouble(
    ...
    final String distance =
        sessionData['distanceKm'] != null
            ...
            : activeData['distance']?.toString() ?? '0.0 km';

    // ----------------------------------------------------------
    // PEE / POOP
    // ----------------------------------------------------------

    final int peeCount = _toInt(
      sessionData['peeCount'] ?? activeData['peeCount'],
    );

    final int poopCount = _toInt(
      sessionData['poopCount'] ?? activeData['poopCount'],
    );
    ...
    final Map<String, dynamic> currentLocation = _mapValue(
      sessionData['currentLocation'],
    );

    final double currentLat = _toDouble(
      currentLocation['lat'] ?? activeData['currentLat'],
    );

    final double currentLng = _toDouble(
      currentLocation['lng'] ?? activeData['currentLng'],
    );
    ...
    final List<dynamic> route = _listValue(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed:
                _isEndingWalk ? null : _showEndWalkDialog,
            icon: const Icon(
  ...
  Widget _completedScreen(
    ...
    final String duration =
        data['duration']?.toString() ?? '00:00:00';

    final String distance =
        data['distance']?.toString() ?? '0.0 km';
