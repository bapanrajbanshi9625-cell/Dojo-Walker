...
class _LiveWalkScreenState extends State<LiveWalkScreen> {
    ...
    try {
      ...
      try {
        ...
        if (data != null) {
          ...
          if (route is List) {
            for (final dynamic item in route) {
              if (item is Map) {
                initialRoute.add(
        ...
        debugPrint(
          'Live session restore error: $e',
        );
      }

      // --------------------------------------------------------
      // START CENTRAL BACKGROUND GPS
      // --------------------------------------------------------
  ...
  void dispose() {
    ...
    // IMPORTANT:
    // Background GPS यहाँ stop नहीं होगा.
    // End Walk flow में ही stop किया जाता है.
